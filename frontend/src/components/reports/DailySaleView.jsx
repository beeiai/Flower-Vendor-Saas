...
const SmsView = ({ customers = [], onCancel, showNotify }) => {
  ...
  const { selectedGroup, selectedCustomer, fromDate, toDate, sending, loading, salesData, totalQty, totalAmount } = state;
  ...
  const fetchDailySales = useCallback(async () => {
    ...
    try {
      ...
      showNotify?.('Failed to load daily sales data', 'error');
  ...
  const handleSendSms = async () => {
    ...
    try {
      ...
      const message = `DAILY SALE REPORT\nPeriod: ${fromDate} to ${toDate}\nCustomer: ${selectedCustomer}\n\nTotal Quantity: ${calculatedTotals.qty.toFixed(2)} kg\nTotal Amount: ₹${calculatedTotals.amount.toFixed(2)}`;
      
      // Send SMS to customer's phone number
      const customer = customers.find(c => c.name === selectedCustomer);
      const phone = customer?.contact || phoneNumber;
      ...
      await api.sendSms({ phoneNumber: phone, message });
    ...
    <div className="flex flex-col h-full bg-slate-50 font-sans text-slate-800 overflow-hidden rounded-2xl shadow-2xl border border-slate-200">
      ...
      <div className="p-4 flex-1 flex flex-col gap-4 overflow-hidden">
        ...
        <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4 flex-wrap">
          ...
          >
            {loading ? 'Loading...' : 'GO'} <ChevronRight size={14} className="group-hover:translate-x-0.5 transition-transform" />
          </button>
        </div>
