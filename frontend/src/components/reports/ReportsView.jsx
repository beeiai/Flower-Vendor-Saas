...
export default function ReportsView({ groups, customers, vehicles, advanceStore = {}, onCancel }) {
	...
	}, [selectedCustomer?.id, autoLoadLocal, setRows]);
	...
	// ✅ FIXED: Navigation starts from GROUP and flows: Group → Vehicle → Customer → Go
	const focusNext = (field) => {
		setTimeout(() => {
			if (field === 'group') vehicleRef.current?.querySelector('input')?.focus();
	...
	const focusPrev = (field) => {
		setTimeout(() => {
			if (field === 'vehicle') groupRef.current?.querySelector('input')?.focus();
		...
		<div className="flex-1 flex flex-col h-full overflow-hidden bg-white" ref={containerRef}>
			...
			<div className="flex-1 flex overflow-hidden bg-slate-50">
				...
				<aside className="w-[340px] bg-gradient-to-b from-slate-800 to-slate-900 flex flex-col p-4 shrink-0 shadow-2xl rounded-r-lg border-l-2 border-[#5B55E6]/30 h-full">
					...
					<div className="space-y-1.5 flex-1 text-white pb-3 overflow-y-auto">
						<div className="bg-slate-700/30 backdrop-blur-sm rounded-lg p-1.5 border border-slate-600/50 shadow-lg shrink-0">
							<label className="text-[10px] font-bold text-slate-300 uppercase tracking-widest mb-0.5 block">Total Qty</label>
							<input type="text" readOnly className="w-full bg-gradient-to-r from-slate-800 to-slate-700 px-2 py-1 text-sm font-black text-right rounded-lg border border-slate-600/50 outline-none text-cyan-400 shadow-inner" value={String(summary.qty)} style={{ colorScheme: 'dark' }} />
						</div>
						<div className="bg-slate-700/30 backdrop-blur-sm rounded-lg p-1.5 border border-slate-600/50 shadow-lg shrink-0">
							<label className="text-[10px] font-bold text-slate-300 uppercase tracking-widest mb-0.5 block">Handling</label>
							<input type="text" readOnly className="w-full bg-gradient-to-r from-slate-800 to-slate-700 px-2 py-1 text-sm font-bold text-right rounded-lg border border-slate-600/50 outline-none text-amber-400 shadow-inner" value={summary.coolie.toFixed(2)} style={{ colorScheme: 'dark' }} />
						</div>
						<div className="bg-slate-700/30 backdrop-blur-sm rounded-lg p-1.5 border border-slate-600/50 shadow-lg shrink-0">
							<label className="text-[10px] font-bold text-slate-300 uppercase tracking-widest mb-0.5 block">Luggage</label>
							<input type="text" readOnly className="w-full bg-gradient-to-r from-slate-800 to-slate-700 px-2 py-1 text-sm font-bold text-right rounded-lg border border-slate-600/50 outline-none text-rose-400 shadow-inner" value={summary.luggageTotal.toFixed(2)} style={{ colorScheme: 'dark' }} />
						</div>
						<div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm rounded-lg p-2 border border-[#5B55E6]/30 shadow-lg shrink-0">
							...
							<div className="grid grid-cols-2 gap-1.5">
								...
								<div className="flex flex-col gap-0.5">
									...
									<input type="text" readOnly className="bg-gradient-to-r from-slate-800/50 to-slate-700/50 px-2 py-0.75 text-right rounded-lg border border-slate-600/30 outline-none text-rose-400 font-bold shadow-inner text-sm" value={summary.totalCommission.toFixed(2)} style={{ colorScheme: 'dark' }} />
								</div>
							</div>
						</div>
						<div className="bg-slate-700/30 backdrop-blur-sm rounded-lg p-1.5 border border-slate-600/50 shadow-lg shrink-0">
							<label className="text-[10px] font-bold text-slate-300 uppercase tracking-widest mb-0.5 block">Gross Total</label>
							<input type="text" readOnly className="w-full bg-gradient-to-r from-slate-800 to-slate-700 px-2 py-1 text-sm font-black text-right rounded-lg border border-slate-600/50 outline-none text-emerald-400 shadow-inner" value={summary.total.toFixed(2)} style={{ colorScheme: 'dark' }} />
						</div>
						<div className="bg-slate-700/30 backdrop-blur-sm rounded-lg p-1.5 border border-slate-600/50 shadow-lg shrink-0">
							<label className="text-[10px] font-bold text-slate-300 uppercase tracking-widest mb-0.5 block">Amount Paid</label>
							<input type="text" readOnly className="w-full bg-gradient-to-r from-slate-800 to-slate-700 px-2 py-1 text-sm font-bold text-right rounded-lg border border-slate-600/50 outline-none text-green-400 shadow-inner" value={summary.paid.toFixed(2)} style={{ colorScheme: 'dark' }} />
						</div>
						<div className="mt-1.5 pt-2 border-t border-white/20 text-center shrink-0">
							...
							<div className="bg-gradient-to-r from-[#5B55E6] to-[#4A44D0] p-2 rounded-lg shadow-xl border border-white/10">
								<p className="text-base font-black text-white tabular-nums drop-shadow-lg">₹ {summary.netTotal.toFixed(2)}</p>
							</div>
						</div>
					</div>
				</aside>
